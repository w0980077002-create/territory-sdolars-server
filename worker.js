    ...
    try{
      if(u.pathname==="/db/test-simple"){
        ...
        try{
          ...
          const columns=info.map(x=>x.name);
          const required=["telegram_id","username","first_name","last_name","level","exp","hp","max_hp","coins","gems","strength","agility","defense","weapon"];
          const missing=required.filter(x=>!columns.includes(x));
          let count=0, count_error="";
          try{ count=Number(this.sql.exec("SELECT COUNT(*) AS count FROM players").toArray()[0]?.count||0); }
          catch(e){ count_error=e?.message||String(e); }
          let sample_count=0, select_error="";
          try{ sample_count=this.sql.exec("SELECT telegram_id FROM players LIMIT 1").toArray().length; }
          catch(e){ select_error=e?.message||String(e); }
          return json({ok:true,diagnostic:"players-query-simple",columns,missing,count,count_error,select_ok:!select_error,select_error,sample_count});
        }catch(e){
          return json({ok:false,diagnostic:"players-query-simple",error:e?.message||String(e)},500);
        }
      }
    ...
    try{
      ...
      if(u.pathname==="/admin/db-test-simple" && request.method==="GET"){
        try{
          ...
          const result=await dbJSON(stub,"/db/test-simple");
          return json(result,200,{"cache-control":"no-store","x-territory-build":"G109-DB-DIAG"});
        }catch(e){
          return json({ok:false,diagnostic:"players-query-simple",error:e?.message||String(e),stage:"worker-call"},500,{"cache-control":"no-store","x-territory-build":"G109-DB-DIAG"});
        }
      }
