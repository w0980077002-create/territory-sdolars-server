  ...
  async fetch(request){
    ...
    try{
      ...
      if(u.pathname==="/db/test"){
        ...
        const missing=required.filter(k=>!info.some(x=>x.name===k));
        let sample=[]; let count=null; let error=null;
        try { count=this.sql.exec("SELECT COUNT(*) AS total FROM players").toArray()[0]?.total ?? null; } catch(e){ error={stage:"count",message:e?.message||String(e)}; }
        if(!error){ try { sample=this.sql.exec("SELECT telegram_id AS id,username,first_name,last_name,level,coins,gems,banned,ban_reason,updated_at,created_at FROM players WHERE username LIKE ? OR first_name LIKE ? OR telegram_id LIKE ? ORDER BY updated_at DESC LIMIT ? OFFSET ?","%","%","%",50,0).toArray(); } catch(e){ error={stage:"select",message:e?.message||String(e),stack:e?.stack||""}; } }
        return json({ok:!error,diagnostic:"players-query",count,missing_columns:missing,columns:info,error,sample_count:sample.length,sample});
      }
      this.init();
  ...
  async fetch(request,env,ctx){
    ...
    try{
      if(u.pathname==="/admin/health" && request.method==="GET"){
        return json({ok:true,service:"admin",version:"G99"},200,{"cache-control":"no-store","x-territory-build":"G99"});
      }
      ...
      if(u.pathname==="/admin/app.js" && request.method==="GET"){
        return new Response(ADMIN_APP_JS,{status:200,headers:{"content-type":"text/javascript; charset=utf-8","cache-control":"no-store","x-territory-build":"G99"}});
      }

      if(u.pathname==="/admin" && request.method==="GET"){
        const auth=await verifyAdminToken(cookies(request)[ADMIN_COOKIE],env);
        return new Response(adminHTML(auth),{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-territory-build":"G99"}});
      }
      ...
      if(u.pathname==="/admin/db-test" && request.method==="GET"){
        try{
          const stub=env.DB.get(env.DB.idFromName("global"));
          return await dbJSON(stub,"/db/test");
        }catch(e){
          return json({ok:false,error:e?.message||String(e),stack:e?.stack||""},500,{"cache-control":"no-store","x-territory-build":"G99"});
        }
      }
      ...
      if(u.pathname==="/admin/db-health" && request.method==="GET"){
        ...
        try{
          ...
          return json({ok:true,diagnostic:"db-health",...result},200,{"cache-control":"no-store","x-territory-build":"G99"});
        }catch(e){
          return json({ok:false,diagnostic:"db-health",error:e?.message||String(e),stack:e?.stack||""},500,{"cache-control":"no-store","x-territory-build":"G99"});
        }
      }
