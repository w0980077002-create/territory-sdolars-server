...
export class TerritoryDB {
  ...
  adminPlayers(q='',limit=50,offset=0){
    ...
    const rowsRaw=this.sql.exec(`SELECT telegram_id AS id,username,first_name,last_name,level,coins,gems,banned,ban_reason,updated_at,created_at FROM players WHERE username LIKE ? OR first_name LIKE ? OR telegram_id LIKE ? ORDER BY updated_at DESC LIMIT ? OFFSET ?`,x,x,x,lim,off).toArray();
    const rows=JSON.parse(JSON.stringify(rowsRaw,(k,v)=>typeof v==='bigint'?Number(v):v));
    const totalRaw=this.sql.exec(`SELECT COUNT(*) total FROM players WHERE username LIKE ? OR first_name LIKE ? OR telegram_id LIKE ?`,x,x,x).toArray()[0]?.total||0;
