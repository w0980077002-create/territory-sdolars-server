...
const authed = !!auth;
return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Territory Admin G96</title>
...
<header><b>⚔️ Territory · G96 Admin</b><span id="status" class="muted">${authed ? ` · ${auth.label}: ${auth.login}` : ""}</span></header><main>
  ...
  async fetch(request,env,ctx){
    ...
    try{
      if(u.pathname==="/admin/health" && request.method==="GET"){
        return json({ok:true,service:"admin",version:"G96"},200,{"cache-control":"no-store","x-territory-build":"G96"});
      }
      ...
      if(u.pathname==="/admin/app.js" && request.method==="GET"){
        return new Response(ADMIN_APP_JS,{status:200,headers:{"content-type":"text/javascript; charset=utf-8","cache-control":"no-store","x-territory-build":"G96"}});
      }

      if(u.pathname==="/admin" && request.method==="GET"){
        const auth=await verifyAdminToken(cookies(request)[ADMIN_COOKIE],env);
        return new Response(adminHTML(auth),{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-territory-build":"G96"}});
      }
