...
/**
 * Territory G111 — Cloudflare Worker + Durable Object SQLite backend
 *
 * Required:
...
const authed = !!auth;
return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Territory Admin G111</title>
...
<header><b>⚔️ Territory · G111 Admin</b><span id="status" class="muted">${authed ? ` · ${auth.label}: ${auth.login}` : ""}</span></header><main>
  ...
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(u.pathname==="/admin/health" && request.method==="GET"){
        return json({ok:true,service:"admin",version:"G111"},200,{"cache-control":"no-store","x-territory-build":"G111"});
      }
      ...
      if(u.pathname==="/admin/app.js" && request.method==="GET"){
        return new Response(ADMIN_APP_JS,{status:200,headers:{"content-type":"text/javascript; charset=utf-8","cache-control":"no-store","x-territory-build":"G111"}});
      }

      if(u.pathname==="/admin" && request.method==="GET"){
        const auth=await verifyAdminToken(cookies(request)[ADMIN_COOKIE],env);
        return new Response(adminHTML(auth),{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-territory-build":"G111"}});
      }
      ...
      if(u.pathname==="/admin/db-test-admin-query" && request.method==="GET"){
        try{
          ...
          return new Response(text,{status:r.status,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store","x-territory-build":"G111"}});
        }catch(e){
          return json({ok:false,error:String(e?.message||e),stage:"worker-call"},500,{"cache-control":"no-store","x-territory-build":"G111"});
        }
      }
      ...
      if(u.pathname==="/admin/db-health" && request.method==="GET"){
        ...
        try{
          ...
          return json({ok:true,diagnostic:"db-health",...result},200,{"cache-control":"no-store","x-territory-build":"G111"});
        }catch(e){
          return json({ok:false,diagnostic:"db-health",error:e?.message||String(e),stack:e?.stack||""},500,{"cache-control":"no-store","x-territory-build":"G111"});
        }
      }
