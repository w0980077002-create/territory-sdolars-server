...
export class TerritoryDB extends DurableObject {
  ...
  arenaReward(id,coins,xp,roomId){
  ...
  async fetch(request){
    ...
    try{
      ...
      if(u.pathname==="/db/arena-reward"){const x=await b();return json({ok:true,player:this.arenaReward(x.id,x.coins,x.xp,x.room_id)});}
