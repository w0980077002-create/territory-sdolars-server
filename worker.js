...
const authed = !!auth;
return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Territory Admin G91.6</title>
...
<header><b>⚔️ Territory · G91.6 Admin</b><span id="status" class="muted">${authed ? ` · ${auth.label}: ${auth.login}` : ""}</span></header><main>
  ...
  async fetch(request,env,ctx){
    ...
    try{
      if(u.pathname==="/admin/health" && request.method==="GET"){
        return json({ok:true,service:"admin",version:"G91.6"},{headers:{"cache-control":"no-store","x-territory-build":"G91.6"}});
      }
      ...
      if(u.pathname==="/admin" && request.method==="GET"){
        ...
        return new Response(adminHTML(auth),{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-territory-build":"G91.6"}});
      }
